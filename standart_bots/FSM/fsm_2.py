...
async def start_command(message: types.Message) -> None:
    await message.answer('Welcome! Type /create for make profile!',
                         reply_markup=get_kb())
...
async def check_age(message: types.Message) -> None:
    await message.reply("It's not a digit, try again!")
...
async def load_description(message: types.Message, state: FSMContext) -> None:
    ...
    await message.reply("Congratulations! Your profile has been created!")
    await state.finish()
